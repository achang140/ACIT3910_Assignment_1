...
try:
    ...
    if db.is_connected():
        ...
        cursor.execute("SELECT * FROM word WHERE word = '" + input_word + "'");  
        ...
        if results:
            ...
            cursor.execute("UPDATE word SET word='" + input_change_word + "' WHERE word='" + input_word + "'");
            db.commit()
            if cursor.rowcount > 0:
                print(f"Changed '{input_word}' to '{input_change_word}' in the database")
        else:
            print(f"The word '{input_word}' was not found... adding")
            cursor.execute("INSERT INTO word (word) VALUES ('" + input_word + "')");
            db.commit()
            if cursor.rowcount > 0:
